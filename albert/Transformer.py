...
class Transformer(nn.Module):
    ...
    def forward(self, input_tensor : torch.Tensor, mask : Optional[torch.BoolTensor] = None) ->torch.Tensor:
        ...
        attn_out = self.multihead_attention(input_tensor,input_tensor,input_tensor,mask)
        in_sub_layer = input_tensor + self.dropout(attn_out)
        in_layer_norm = self.layer_norm(in_sub_layer)
        position_wise_ff_outputs = self.position_wise_ff_out(self.gelu(self.position_wise_ff_in(in_layer_norm)))
        out_sub_layer = in_layer_norm + self.dropout(position_wise_ff_outputs)

        output_layer_norm = self.layer_norm(out_sub_layer)
        return self.dropout(output_layer_norm)